...
colnames(dat) <- c("chr", "start", "stop", "counts_tel", "gc_tel")
dat <- dat %>% filter(chr %in% c(as.character(1:22),"X"))

## GC-normalize
## 1. Create GC bins
dat$gc_grp <- cut(dat$gc_tel, breaks = c(0, 0.3, 0.35, 0.40, 0.45, 0.50, 0.55, 0.65))
...
colnames(dat2) <- c("chr", "start", "stop", "counts_tx", "gc_tx", "prop_tx")
...
dat3 <- dat %>% left_join(dat2,by=c("chr","start","stop")) # BUG: The 'by=' argument is neccessary. Or the left_join can't work
